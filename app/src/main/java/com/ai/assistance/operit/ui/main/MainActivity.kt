...
import com.ai.assistance.operit.util.car.CarRuntimeOptimizer
import java.util.*
...
class MainActivity : ComponentActivity() {
    ...
    private lateinit var carPreferencesManager: CarPreferencesManager
    ...
    private lateinit var carRuntimeOptimizer: CarRuntimeOptimizer
    ...
    private fun initializeComponents() {
        ...
        // 初始化车机偏好管理器并设置默认主界面
        carPreferencesManager = CarPreferencesManager.getInstance(this)
        val mainMode = carPreferencesManager.getMainInterfaceModeSync()
        if (mainMode == CarPreferencesManager.MainInterfaceMode.VOICE_FLOATING) {
            currentMainNavItem = NavItem.AiChat
        }

        // 初始化车机运行时优化
        carRuntimeOptimizer = CarRuntimeOptimizer(this, lifecycleScope)
        carRuntimeOptimizer.applyToActivity(this)
        carRuntimeOptimizer.applyStartupOptimization()
                ...
                Box {
                    // 检查是否需要显示用户协议
                        ...
                        // 显示主应用界面
                            ...
                            // 车机模式：检查是否使用语音悬浮窗作为主界面
                            val carMainMode = carPreferencesManager.getMainInterfaceModeSync()
                            if (carMainMode == CarPreferencesManager.MainInterfaceMode.VOICE_FLOATING &&
                                pendingShortcutNavItem == null && pendingRouteId.isNullOrBlank()
                            ) {
                                // 车机语音悬浮窗主界面模式
                                CompositionLocalProvider(LocalPluginLoadingState provides pluginLoadingState) {
                                    com.ai.assistance.operit.ui.features.car.CarHomeScreen(
                                        onNavigateToChat = {
                                            currentMainNavItem = NavItem.AiChat
                                            setAppContent()
                                        },
